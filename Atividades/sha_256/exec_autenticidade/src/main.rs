...
// Lê um arquivo do sistema e retorna seu hash SHA-256
fn processar_arquivo(caminho: &str) -> io::Result<String> {
    let bytes = fs::read(caminho)?;
    Ok(sha256(&bytes))
}

// Verifica se o Hash do arquivo coincide com o Hash fornecido
fn verificar_autenticidade(caminho: &str, hash_esperado: &str) -> bool {
    match processar_arquivo(caminho){
        Ok(hash_gerado) => {
            hash_gerado.trim().to_lowercase() == hash_esperado.trim().to_lowercase()
        }
        Err(e) => {
...
fn main(){
    loop {
        ...
        println!("1. Gerar Hash do arquivo arquivo");
        println!("2. Verificar autenticidade");
        println!("3. Sair");
        ...
        match opcao.trim(){
            ...
            "2" => {
                ...
                print!("Digite o hash (chave) para verificação: ");
                io::stdout().flush().unwrap();
                let mut hash_fornecido = String::new();
                io::stdin().read_line(&mut hash_fornecido).unwrap();

                if verificar_autenticidade(caminho.trim(), hash_fornecido.trim()) {
                    println!("\n✅ VÁLIDO: O arquivo é autêntico e está íntegro.");
                } else {
                    println!("\n❌ INVÁLIDO: O arquivo foi alterado ou a chave é inválida.");
                }
            }
            "3" => break,
            _ => println!("\nOpção Inválida!"),
