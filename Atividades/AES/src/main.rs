...
fn shift_rows(state: &mut[u8; 16]){
    // Coluna 1
    let temp = state[1]; // --- Salva primeiro elemento
    ...
    // Coluna 2
    let temp1 = state[2]; // --- Salva primeiro elemento
    let temp2 = state[6]; // --- Salva segundo elemento
    ...
    // Colina 3
    let temp = state[15]; // --- Salva quarto elemento
...
fn main(){
    ...
    println!("Digite a senha de 32 caracteres: ");
    ...
    let senha = senha.trim(); // ---Remove espaços e quebras de linha
        ...
        // ---Verifica se a senha tem 32 bytes / 32 caracteres
    if senha.as_bytes().len() != 32 {
        println!("[!] ERRO: A senha deve ter exatamente 32 caracteres.");
        return;
    }

        // ---Converte a senha para um array de 32 bytes
    let mut key = [0u8; 32];
    key.copy_from_slice(&senha.as_bytes()[..32]);

    // 2. Expansão da chave
    let round_keys = key_expansion(&key);

    // 3. Leitura do arquivo "mensagem.txt"
    //let mut block = [0u8; 16];
    let mut file_in = File::open("mensagem.txt").expect("Não foi possível abrir o arquivo.");
    let mut file_out = File::create("mensagem_criptografada.txt").expect("Falha ao criar arquivo");

    //file_in.read_exact(&mut block).expect("O arquivo deve ter exatamente 16 bytes.");
    //println!("Conteúdo original (hex): {:02x?}", block);

    // 4. === Criptografia ===
    let mut buffer = [0u8; 16];

    println!("Criptografando arquivo...");

    while let Ok(n) = file_in.read(&mut buffer){
        if n == 0 { break; } // Fim do arquivo

        add_round_key(&mut buffer, &round_keys[0]); // Adição da chave inicial
        ...
        for i in 1..14 {
            ...
            add_round_key(&mut buffer, &round_keys[i]);
        }

            // ---rodada final (sem mixcolumn)
        sub_bytes(&mut buffer);
        shift_rows(&mut buffer);
        add_round_key(&mut buffer, &round_keys[14]);

        // 5. Salvar resultado .txt
        file_out.write_all(&buffer).expect("Falha ao escrever um arquivo.");
    }
    println!("Criptografado com sucesso em 'mensagem_criptografada.txt'!");

    // Leitura do arquivo criptografado
    let mut file_in_cript = File::open("mensagem_criptografada.txt").expect("Não foi possível abrir o arquivo.");

    let mut file_out = File::create("mensagem_descriptografada.txt").expect("Falha ao criar arquivo");

    // 6. === Descriptografia ===
    println!("Descriptografando arquivo...");

    while let Ok(n) = file_in_cript.read(&mut buffer) {
        if n == 0 { break; } // Fim do arquivo

        add_round_key(&mut buffer, &round_keys[14]); // Adição da chave inicial
        ...
        inv_sub_bytes(&mut buffer);
        inv_shift_rows(&mut buffer);
        add_round_key(&mut buffer, &round_keys[0]);

            // 7. Salvar resultado .txt
        file_out.write_all(&buffer).expect("Falha ao escrever um arquivo.");
    }
    println!("Descriptografado com sucesso em 'mensagem_descriptografada.txt'!");
}
