use std::io::{self, Write}; // Biblioteca para entrada e saída
...
fn criptografar(block: &mut [u8; 16], round_keys: &[[u8; 16];15]){
    add_round_key(block, &round_keys[0]); // Rodada inicial

    // 14 rodadas principais (1 a 13)
    for round in 1..13 {
        sub_bytes(block);
        shift_rows(block);
        mix_columns(block);
...
fn main(){
    ...
    print!("Digite a senha (32 bytes / 32 caracteres): ");
    io::stdout().flush().unwrap(); // Força exibição

    let mut senha = String::new();
    io::stdin().read_line(&mut senha).expect("Falha ao ler senha.");
    let senha = senha.trim(); // Remove espaços e quebras de linha

    // Verifica se a senha tem 32 bytes
    ...
    println!("Senha configurada com sucesso!");
    println!("Chave (hex): {}", hex::encode(&key));
    println!();

    // Expade a chave para gerar 15 chaves das rodadas
    println!("Expandindo chave para 15 rodadas...");
    let round_keys = key_expansion(&key);
    println!("Expansão concluída!");

    // Texto de exemplo para criptografar
    let mut block = [0u8; 16];
    let plaintext = b"Exemplo AES-256!";
    block.copy_from_slice(&plaintext[..16]);

    println!("Original: {:02x?}", block);
    criptografar(&mut block, &round_keys);
    println!("Criptografado: {:02x?}", block);
}
