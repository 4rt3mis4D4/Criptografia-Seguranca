...
use std::fs::{File, OpenOptions}; // Biblioteca para manipulação de arquivos
use std::io::{Read, Write, BufReader}; // Biblioteca para leitura e escrita de arquivos + buffer
...
fn criptografar(caminho: &str){
    ...
    let salt = [0u8; 16];
    let iteracoes = 10000;
    let chave_mestra = pbkdf2(senha_limpa, &salt, iteracoes);

    // --- Expansão da chave 
    let chaves_rodada = key_expansion(&chave_mestra);

    // --- Abrir arquivo
    let arquivo = File::open(caminho).expect("Falha ao abrir arquivo...");
    let mut leitor = BufReader::new(arquivo);
    let mut buffer = [0u8; 16];

    // --- Criação do arquivo para armazenar criptografia
    let mut arquivo_criptografado = File::create("texto_criptografado.txt").expect("Falha ao criar arquivo para criptografia...");


    println!("\nCriptografando...");

    // --- Processamento de blocos
    while let Ok(n) = leitor.read(&mut buffer){
        if n == 0 {
            break;
        }

        // --- Rodadas AES-256: INICIAL
        add_round_key(&mut buffer, &chaves_rodada[0]);

        // --- Rodadas AES-256: 1 a 13
        for i in 1..14 {
            sub_bytes(&mut buffer);
            shift_rows(&mut buffer);
            mix_columns(&mut buffer);
            add_round_key(&mut buffer, &chaves_rodada[i]);
        }

        // --- Rodadas AES-256: FINAL
        sub_bytes(&mut buffer);
        shift_rows(&mut buffer);
        add_round_key(&mut buffer, &chaves_rodada[14]);

        // --- Salva criptografia em um arquivo
        arquivo_criptografado.write_all(&buffer).expect("Falha ao escrever no arquivo criptografado...");
    }
...
fn descriptografar(caminho: &str){
    // --- Solicita senha ao usuário
    print!("\nDigite a mesma senha utilizada na criptografia do arquivo: ");
    io::stdout().flush().unwrap();
    ...
    // --- Abrir Arquivo
    let arquivo_criptografado = File::open(caminho).expect("Falha ao abrir o arquivo criptografado...");
    let mut leitor = BufReader::new(arquivo_criptografado);
    
    // --- Criação do arquivo descriptografado
    let mut arquivo_descriptografado = File::create("texto_descriptografado.txt").expect("Falha ao criar arquivo descriptografado...");
    
    let mut buffer = [0u8; 16];

    println!("\nDescriptografando...");

    // --- Processamento de blocos na ordem inversa
    while let Ok(n) = leitor.read(&mut buffer) {
        if n == 0 {
            break;
        }

        // --- Rodadas AES-256 Inversa: INICIAL (rodada 14)
        add_round_key(&mut buffer, &chaves_rodada[14]);

        // --- Rodadas AES-256 Inversa: 13 a 1
        for i in(1..14).rev() {
        ...
        // --- Rodadas AES-256 Inversa: FINAL
        inv_shift_rows(&mut buffer);
        inv_sub_bytes(&mut buffer);
        add_round_key(&mut buffer, &chaves_rodada[0]);

        // --- Salva a descriptografia em um arquivo
        arquivo_descriptografado.write_all(&buffer).expect("Falha ao escrever no arquivo descriptografado...");
    }
...
fn main(){
    loop {
        println!("=== AES-256 ===");
        println!("(1) Criptografar");
        println!("(2) Descriptografar");
        println!("(3) Sair");
        ...
        match opcao.trim() {
            "1" => {
                let arq_entrada = "texto.txt";
                
                if std::path::Path::new(arq_entrada).exists() {
                    criptografar(arq_entrada);
                } else {
                    eprintln!("\nArquivo {} não encontrado.\n", arq_entrada);
                }
            }
            "2" => {
                let arq_entrada = "texto_criptografado.txt";

                if std::path::Path::new(arq_entrada).exists() {
                    descriptografar(arq_entrada);
                } else {
                    eprintln!("\nArquivo {} não encontrado.\n", arq_entrada);
                }
            }
